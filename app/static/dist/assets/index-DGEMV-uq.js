...
`,S0=`/* An aggregate with four joins */
SELECT c.state,
  cat.categoryname,
  sum(o.netamount),
  sum(o.totalamount)
...
`,k0=`"Limit  (cost=1.27..3878.21 rows=5 width=172) (actual time=0.245..2.544 rows=5 loops=1)"
...
"Execution Time: 2.900 ms"`,T0=String.raw`
                                                                         QUERY PLAN
═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
...
`,E0=`Append (actual time=102.409..302.675 rows=3 loops=1)
  CTE init
...
`,g0=` Merge on public.tb  (cost=262602.88..306621.45 rows=0 width=0) (actual time=7798.118..7798.122 rows=0 loops=1)
...
(112 lignes)
`,A0=`EXPLAIN (ANALYZE,BUFFERS,VERBOSE,SETTINGS,WAL,SERIALIZE,MEMORY,FORMAT TEXT)
WITH
...
order by
        l_shipmode
LIMIT 1;
`,b0=` Limit  (cost=1964755.66..1964961.44 rows=1 width=27) (actual time=7579.592..7922.997 rows=1 loops=1)
 ...
 Execution Time: 7923.770 ms`,N0=`                                                                                            QUERY PLAN
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
(26 rows)`,O0="DELETE FROM emailmessages where emailmessageid in ( select emailmessageid from emailmessages limit 5000 );",x0=`
